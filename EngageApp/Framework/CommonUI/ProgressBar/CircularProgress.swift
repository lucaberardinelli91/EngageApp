...
public class CircularProgress: UIView, CAAnimationDelegate {
    ...
    public var glowAmount: CGFloat = 1.0 {
        didSet {
            glowAmount = glowAmount.clamp(lowerBound: 0.0, upperBound: 3.0)
            progressLayer.glowAmount = glowAmount
        }
    }

    public var glowMode: CircularProgressGlowMode = .constant {
        didSet {
    ...
    private class CircularProgressViewLayer: CALayer {
        ...
        var lerpColorMode = false
        var gradientRotateSpeed: CGFloat = 0.0 {
            didSet { invalidateGradientCache() }
        }

        var glowAmount: CGFloat = 0.0
        var glowMode: CircularProgressGlowMode = .constant
        ...
        private enum GlowConstants {
            private static let sizeToGlowRatio: CGFloat = 0 // 0.00015
            static func glowAmount(forAngle _: Double, glowAmount: CGFloat, glowMode: CircularProgressGlowMode, size: CGFloat) -> CGFloat {
                switch glowMode {
                case .constant:
        ...
        private func drawGradient(withContext context: CGContext, colorsArray: [UIColor]) {
            ...
            if let cachedGradient = gradientCache {
                ...
                guard
                    let newGradient = CGGradient(
                        colorSpace: baseSpace,
                        colorComponents: colorsArray.rgbNormalized.componentsJoined,
                        locations: locations,
...
private extension BinaryFloatingPoint {
    ...
    func colorLerp(minColor: UIColor, maxColor: UIColor) -> UIColor {
        let clampedValue = CGFloat(clamp(lowerBound: 0.0, upperBound: 1.0))
        ...
        return UIColor(
            red: clampedValue.lerp(min: r0, max: r1),
